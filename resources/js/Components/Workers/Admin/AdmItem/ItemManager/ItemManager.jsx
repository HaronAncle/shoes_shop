...
const ItemManager = () => {
    ...
    const fetchItems = async () => {
        try {
            const response = await axios.get(`/api/workpanel/admin/items`);
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`/api/workpanel/admin/items/${id}`);
            fetchItems();
        } catch (error) {
            console.error(
                "Error deleting item:",
    ...
    const handleSave = async (formData) => {
        try {
            if (modalType === "edit") {
                await axios.put(
                    `/api/workpanel/admin/items/${selectedItem.id}`,
                    formData,
                    { headers: { "Content-Type": "multipart/form-data" } }
                );
            } else {
                await axios.post(`/api/workpanel/admin/items`, formData, {
                    headers: { "Content-Type": "multipart/form-data" },
                });
                setCurrentPage(1); // Отправить на первую страницу после добавления
            }
            fetchItems();
            setShowModal(false);
        } catch (error) {
